...
const AptitudeExam = () => {
  ...
  const fetchInterview = useCallback(async () => {
    try {
      ...
      setInterview(data);
      // If aptitude is already submitted, redirect back
      const aptQs = (data.questions || []).filter(q => q.round === 'aptitude');
      if (aptQs.length > 0 && aptQs.every(q => q.answer)) {
        setSubmitted(true);
      }
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const aptQs = (interview.questions || []).filter(q => q.round === 'aptitude');
    if (aptQs.length === 0 || aptQs.every(q => q.answer)) return;
  ...
  const handleAutoSubmit = async (aptQs) => {
    try {
      const promises = aptQs
  ...
  const aptQs = (interview.questions || []).filter(q => q.round === 'aptitude');
  const answered = aptQs.filter(q => q.answer || mcqSelections[q._id]).length;
  const isExpired = timeLeft === 0;
  const isUrgent = timeLeft !== null && timeLeft < 300;

  /* ── submitted screen ── */
  if (submitted || aptQs.every(q => q.answer)) {
    return (
      <div className="max-w-2xl mx-auto py-16 text-center space-y-6">
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center mx-auto shadow-xl shadow-violet-200">
          <CheckCircleIcon className="h-12 w-12 text-white" />
        </div>
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">Aptitude Round Complete!</h1>
          <p className="text-gray-500 mt-2">Your answers have been submitted. Results will be available after evaluation.</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-left space-y-3">
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Total Questions</span>
            <span className="font-bold text-gray-900">{aptQs.length}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Answered</span>
            <span className="font-bold text-emerald-600">{aptQs.filter(q => q.answer).length}</span>
          </div>
          <div className="flex justify-between text-sm">
        ...
        <button onClick={() => navigate('/app/user/interviews')}
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white font-bold hover:opacity-90 transition-opacity shadow-lg">
  ...
  return (
    <div className="space-y-6 pb-16">
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-base font-bold text-gray-900 leading-none">Aptitude Round</h1>
                <p className="text-xs text-gray-400 mt-0.5">{answered}/{aptQs.length} answered</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5">
              <div className="w-24 bg-gray-200 h-1.5 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-violet-500 to-purple-600 transition-all"
                  style={{ width: `${(answered / aptQs.length) * 100}%` }} />
              </div>
              <span className="text-xs font-bold text-gray-700">{Math.round((answered / aptQs.length) * 100)}%</span>
            </div>
            ...
            <button onClick={() => setShowConfirm(true)} disabled={answered === 0}
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white text-sm font-bold hover:opacity-90 transition-opacity disabled:opacity-40 shadow">
              Submit Exam
            </button>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        {/* Exam banner */}
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-2xl p-5 flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center flex-shrink-0">
            <PuzzlePieceIcon className="h-5 w-5 text-white" />
          </div>
          <div>
            <h2 className="font-bold text-violet-900">Aptitude Exam — {aptQs.length} MCQ Questions</h2>
            <p className="text-violet-700 text-sm mt-0.5">45 minutes · Logical reasoning · Quantitative aptitude · Pattern recognition</p>
          </div>
          {isUrgent && !isExpired && (
          ...
          return (
            <div key={q._id} className={`bg-white rounded-2xl border p-6 transition-all ${
              isAnswered
                ...
                : selected ? 'border-violet-300 shadow-md' : 'border-gray-100 shadow-sm hover:border-gray-200'
            }`}>
              {/* Q header */}
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 text-white text-sm font-bold flex items-center justify-center flex-shrink-0">
                    {i + 1}
                  </span>
                  ...
                  <span className="px-2 py-0.5 rounded text-xs bg-violet-50 text-violet-600 font-medium">
                    {q.category}
                  </span>
                </div>
                    ...
                    <label key={oi}
                      className={`flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${
                        isAnswered
                          ...
                          : isSelected ? 'border-violet-400 bg-violet-50 shadow-sm' : 'border-gray-200 hover:border-violet-300 hover:bg-violet-50/50'
                      }`}>
                      <input
                        type="radio"
                        name={`q-${q._id}`}
        ...
        <div className="flex justify-end pt-4">
          <button onClick={() => setShowConfirm(true)} disabled={answered === 0 || submitting}
            className="flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white font-bold hover:opacity-90 transition-opacity disabled:opacity-40 shadow-lg shadow-violet-200">
            <PaperAirplaneIcon className="h-5 w-5" />
            Submit Aptitude Exam
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl text-center space-y-5">
            <div className="w-14 h-14 rounded-2xl bg-violet-100 flex items-center justify-center mx-auto">
              <PaperAirplaneIcon className="h-7 w-7 text-violet-600" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-gray-900">Submit Aptitude Exam?</h3>
              <p className="text-gray-500 text-sm mt-1">
                You've answered <strong>{answered}/{aptQs.length}</strong> questions. Once submitted you cannot change your answers.
              </p>
            </div>
            {answered < aptQs.length && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 font-medium">
                ⚠️ {aptQs.length - answered} question{aptQs.length - answered > 1 ? 's' : ''} unanswered — they will be marked incorrect.
              </div>
            )}
            <div className="flex gap-3">
              ...
              <button onClick={handleSubmit} disabled={submitting}
                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white font-bold hover:opacity-90 transition-opacity disabled:opacity-60">
